	...
	while (curr_node != NULL)
	{
		if (is_valid_env(curr_node->cmd) == TRUE)
		{
			...
			if (loc != FALSE)
			{
				...
				{
					free(g_env_list.envp[loc]);
					g_env_list.envp[loc] = ft_strdup(g_env_list.envp[loc + 1]);
					loc++;
				}
				free(g_env_list.envp[envp_cnt() - 1]);
				g_env_list.envp[envp_cnt() - 1] = NULL;
			}
		}
...
int	is_str_in_envp(char *str)
{
	...
	{
		if (ft_strncmp(str, g_env_list.envp[i], ft_strlen(str)) == 0)
			return (i);
	}
	return (FALSE);
}
